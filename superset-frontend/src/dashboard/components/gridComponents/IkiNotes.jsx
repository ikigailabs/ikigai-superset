...
/**
 * COMPONENT: Notes
 * WIDGET FRONTEND URL EXAMPLE: http://localhost:3000/widget/notes?mode=preview&title=Deepcast Test App&items=ai,deepcast,ecommerce,planning,inventory&icon=urlencodedbase64
 * PARAMETERS:
 * mode=preview
 * title=Deepcast Test App
 * items=ai,deepcast,ecommerce,planning,inventory
 */
...
class IkiNotes extends React.PureComponent {
    constructor(props) {
        super(props);
        this.state = {
            isFocused: false,
        ...
        if (!prevProps.editMode && this.props.editMode) {
            if (
                document.getElementById(
                    `ikinotes-widget-${this.props.component.id}`,
                )
            ) {
                ...
                const iframeSrc = this.props.ikigaiOrigin + widgetUrl.pathname + widgetUrl.search;

                const tempIframe = `<iframe
                id="ikinotes-widget-${this.props.component.id}"
                name="ikinotes-${timestamp}"
                src="${iframeSrc}"
            ...
            if (event.origin === this.props.ikigaiOrigin) {
                ...
                if (messageObject.info && messageObject.dataType) {
                    ...
                    if (
                        ...
                        widgetUrl = `${this.props.ikigaiOrigin}/widget/notes?mode=edit`;
                    }
    ...
    renderIframe() {
        ...
        if (ikigaiOrigin) {
            if (markdownSource) {
                ...
                iframeSrc = `${ikigaiOrigin}/widget/notes?mode=edit`;
            }
...
IkiNotes.propTypes = propTypes;
IkiNotes.defaultProps = defaultProps;
...
export default connect(mapStateToProps)(IkiNotes);